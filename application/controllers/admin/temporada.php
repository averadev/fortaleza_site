 ...
 */
    ...
    public function index() {
        $arr['page'] = 'temporada';
        $this->load->view('admin/vwTemporada',$arr);
    }
    ...
    public function get(){
        if($this->input->is_ajax_request()){
            $data = $this->temporada_db->get($_POST['id']);
    ...
    public function delete(){
        if($this->input->is_ajax_request()){
            $data = $this->temporada_db->delete($_POST['id']);
            echo json_encode($data);
        }
    }
    ...
    public function getSearch(){
        if($this->input->is_ajax_request()){
            ...
            $pagina = $_POST['pagina'];
            $total = $this->temporada_db->getCount($_POST['texto'])[0];
            $data = $this->temporada_db->getSearch($_POST['texto'], $pagina);
            // Formato a fechas
            foreach ($data as $obj){
    ...
    public function save(){
        if($this->input->is_ajax_request()){
            ...
            $data = $this->temporada_db->verifyDates($_POST['fechaInicio'], $_POST['fechaFin'], $_POST['id']);
            ...
            if  (count($data) > 0){
                ...
                if ($_POST['id'] == '0'){
                    unset($_POST['id']);
                    $_POST['id'] = $this->temporada_db->insert($_POST);
                }else{
                    $this->temporada_db->update($_POST);
                }
    ...
    public function saveHabitacion(){
        if($this->input->is_ajax_request()){
            ...
            $this->temporada_db->deleteRoom($_POST['id']);
            ...
            for ($i = 0; $i < count($rooms); $i++) {
                if (is_numeric($arrayStd[$i]) || 
                   ...
                   is_numeric($arrayNino[$i])){
                    ...
                    $update = array('temporadaId'=>$_POST['id'], 
                    ...
                    $this->temporada_db->saveRoom($update);
                }
    ...
    public function getHabitaciones(){
        if($this->input->is_ajax_request()){
            $data = $this->temporada_db->getHabitaciones($_POST['id']);
            echo json_encode($data);
        }
    }
